...
set_MVN <- c(1,15)
res <- list()
for (i in 1:length(set_MVN)) {
    res[[i]] <- readRDS(paste0("rand_samples_", set_MVN[i], ".rds"))
...
p <- which(params[set_MVN[1],] != params[set_MVN[2],])
dat <- list()
plot_max <- 0
plot_max_V <- 0
for (i in 1:length(set_MVN)) {
...
#===========================
# plot
fig_file <- "figures/simulations_mvn_10k_sapply.pdf"
...
#legend(-0.03, mylims[2], legend = params[set_MVN, p],
#       col = cols, lty = rep(1, length(set_MVN)), title = expression(alpha))

par(mar = c(0,0,3,0.5))
plot.new()
legend(0, 0.8, legend = paste0(c("exponential","multivar. normal"), "\n(d = ", params[set_MVN, 12],")\n"),
       col = cols, lty = rep(1, length(set_MVN)), title = "Distrib. mut. effect",
       cex = 1.5)
#legend(0, 0.5, legend = params[set_MVN, 12],
#       col = cols, lty = rep(1, length(set_MVN)), title = "d",
#       cex = 1.5)
...
for (j in 1:length(set_MVN)){
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    V <- dat[[j]]$zbv/dat[[j]]$d^2/dat[[j]]$v_sum^2 * params$n[set_MVN[j]]
    lines(xx, V, col = cols[j])
}
lines(t,2*f0*f0, col='black', lty='dotted')

mtext("Distribution of mutation effects", side = 3, line = -2, outer = T, cex = 1.5)
mtext("e)", side = 3, line = -2.2, adj = 0.05, outer = T, cex = 2)
