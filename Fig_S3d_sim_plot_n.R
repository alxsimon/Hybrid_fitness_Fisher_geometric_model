...
#===========================
# plot
fig_file <- "figures/simulations_n_10k_sapply.pdf"
...
for (j in 1:length(set_n)){
    ...
    if (n > 1){
        hist(c(as.matrix(dat[[j]]$m))/sqrt(dat[[j]]$var_m),
             ...
             ylab='Pr. density', freq=F, xlim=4*c(-1,1), ylim=c(0,0.7),
...
for (j in 1:length(set_n)){
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    V <- dat[[j]]$zbv/dat[[j]]$d^2/dat[[j]]$v_sum^2 * params$n[set_n[j]]
    lines(xx, V, col = cols[j])
}
lines(t,2*f0*f0, col='black', lty='dotted')

mtext("Number of traits", side = 3, line = -2, outer = T, cex = 1.5)
mtext("d)", side = 3, line = -2.2, adj = 0.05, outer = T, cex = 2)
