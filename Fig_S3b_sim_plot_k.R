...
set_k <- c(1,2,3)
res <- list()
for (i in 1:length(set_k)) {
    res[[i]] <- readRDS(paste0("rand_samples_", set_k[i], ".rds"))
...
p <- which(params[set_k[1],] != params[set_k[2],])
dat <- list()
plot_max <- 0
plot_max_V <- 0
for (i in 1:length(set_k)) {
...
#===========================
# plot
fig_file <- "figures/simulations_k_10k_sapply.pdf"
...
#legend(-0.03, mylims[2], legend = params[set_k, p],
#       col = cols, lty = rep(1, length(set_k)), title = expression(alpha))

par(mar = c(0,0,3,0.5))
plot.new()
legend(0, 0.8, legend = paste0(params[set_k, p], " (d = ", params[set_k,12],")"),
       col = cols, lty = rep(1, length(set_k)), title = expression(alpha),
       cex = 1.5)
#legend(0.5, 0.8, legend = params[set_k, 12],
#       col = cols, lty = rep(1, length(set_k)), title = "d",
#       cex = 1.5)
...
for (j in 1:length(set_k)){
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    V <- dat[[j]]$zbv/dat[[j]]$d^2/dat[[j]]$v_sum^2 * params$n[set_k[j]]
    lines(xx, V, col = cols[j])
}
lines(t,2*f0*f0, col='black', lty='dotted')

mtext("Curvature of the fitness function", side = 3, line = -2, outer = T, cex = 1.5)
mtext("b)", side = 3, line = -2.2, adj = 0.05, outer = T, cex = 2)
