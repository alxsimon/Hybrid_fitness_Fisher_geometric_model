...
set_core <- c(1)
res <- list()
for (i in 1:length(set_core)) {
    res[[i]] <- readRDS(paste0("rand_samples_", set_core[i], ".rds"))
...
params$d <- NA
disp_params <- c(6,1,2,4,5,9,12)
text_params <- c(expression(alpha),
                 "N",
                 "n",
                 "opt 1",
                 "opt 2",
...
p <- which(params[set_core[1],] != params[set_core[2],])
dat <- list()
plot_max <- 0
plot_max_V <- 0
for (i in 1:length(set_core)) {
    ...
    #z_bridge <- matrix(unlist(res[[i]]),
    #                   nrow = d+1, ncol = length(res[[i]]),
    #                   byrow = F)
    z_bridge <- res[[i]]
...
#===========================
# plot
fig_file <- "figures/simulations_core_10k_sapply.pdf"
...
#legend(-0.03, mylims[2], legend = params[set_core, p],
#       col = cols, lty = rep(1, length(set_core)), title = expression(alpha))

par(mar = c(0,0,3,0.5))
plot.new()
#legend(0, 0.8, legend = params[set_core, p],
#       col = cols, lty = rep(1, length(set_core)), title = expression(alpha))
#legend(0.5, 0.8, legend = params[set_core, 12],
#       col = cols, lty = rep(1, length(set_core)), title = "d")
text2add <- matrix(c(text_params, as.character(params[i, disp_params])), 7, 2)
text2add[13] <- "exp."
sapply(1:2, function(i) text((i/2)-0.2, (1:7)/7, rev(text2add[,i]), cex = 1.5))
...
for (j in 1:length(set_core)){
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    V <- dat[[j]]$zbv/dat[[j]]$d^2/dat[[j]]$v_sum^2 * params$n[set_core[j]]
    lines(xx, V, col = cols[j])
}
lines(t,2*f0*f0, col='black', lty='dotted')

mtext("Core parameters", side = 3, line = -2, outer = T, cex = 1.5)
mtext("a)", side = 3, line = -2, adj = 0.05, outer = T, cex = 2)
