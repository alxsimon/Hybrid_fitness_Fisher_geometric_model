...
for (i in 1:length(set_N)) {
    res[[i]] <- readRDS(paste0("rand_samples_", set_N[i], ".rds"))
...
p <- which(params[set_N[1],] != params[set_N[2],])
dat <- list()
plot_max <- 0
plot_max_V <- 0
for (i in 1:length(set_N)) {
...
#===========================
# plot
fig_file <- "figures/simulations_bigN_10k_sapply.pdf"
...
for (j in 1:length(set_N)){
    ss <- round(seq(1,dat[[j]]$d+1,length.out = 1000))
    ssx <- (ss-1)/dat[[j]]$d
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    lines(xx, dat[[j]]$zs/dat[[j]]$d/dat[[j]]$v_sum, col = cols[j])
    lines(ssx, dat[[j]]$zb[ss]/dat[[j]]$d/dat[[j]]$v_sum, col = cols[j])
}
lines(t,f0,col='black',lty='dotted')
#legend(-0.03, mylims[2], legend = params[set_N, p],
#       col = cols, lty = rep(1, length(set_N)), title = expression(alpha))

par(mar = c(0,0,3,0.5))
plot.new()
legend(0, 0.8, legend = paste0(params[set_N, p], " (d = ", params[set_N,12],")"),
       col = cols, lty = rep(1, length(set_N)), title = "N",
       cex = 1.5)
#legend(0.5, 0.8, legend = params[set_N, 12],
#       col = cols, lty = rep(1, length(set_N)), title = "d",
#       cex = 1.5)
...
for (j in 1:length(set_N)){
    ...
    ssx <- (ss-1)/dat[[j]]$d
    xx <- (0:dat[[j]]$d)/dat[[j]]$d
    V <- dat[[j]]$zbv[ss]/dat[[j]]$d^2/dat[[j]]$v_sum^2 * params$n[set_N[j]]
    lines(ssx, V, col = cols[j])
}
lines(t,2*f0*f0, col='black', lty='dotted')

mtext("Population size", side = 3, line = -2, outer = T, cex = 1.5)
mtext("c)", side = 3, line = -2, adj = 0.05, outer = T, cex = 2)
